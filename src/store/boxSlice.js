...
const initialState = {
  ...
  selectedSteps: [{ id: 1, title: 'Step - 1', subText: 'Hello world' }],
  paymentStatus: '',
};
...
const boxSlice = createSlice({
  ...
  reducers: {
    increment: (state, action) => {
      ...
      const filter = state.boxes.filter((m) => {
        if (m.id === singleData.id) {
          m.howMany += 1;
          console.log('m.howMany', m.howMany);
          return m;
        }
      });
    ...
    removeFromCart: (state, action) => {
      state.boxes.map((item) =>
        item.id === action.id ? { ...item, selected: false, howMany: 1 } : item
      );
    },
    addQuantity: (state, action) => {
      state.boxes.map((item) =>
        item.id === action.id ? { ...item, howMany: item.howMany + 1 } : item
      );
    },
    ...
    emptyCart: (state) => {
      state.boxes.map((item) =>
        item.selected ? { ...item, selected: false, howMany: 1 } : item
      );
    },
